...
```
repo (facial-academy-design-system)
...
└── design-system/                   # PACOTE consumível
    ...
    ├── facial-academy-design-system.css     # DROP-IN: tokens + reset + foco + motion + tipografia + botões + chips/status
    ├── facial-academy-design-tokens.json    # Tokens legíveis por máquina (Framer, Style Dictionary-like, IA)
    ...
    └── copy-deck.facial-academy.json        # Strings de produto do showcase
```

**Duas camadas de classe (importante):**
- **Pacote drop-in** usa classes prefixadas **`fc-*`** (`.fc-btn`, `.fc-fill`, `.fc-h1`, `.fc-status`…). É o que você consome num projeto.
...
3. **Escolha do usuário** persistida em `localStorage['fc-theme']` (`'light'`|`'dark'`), que vence o sistema.
...
| Token | Hex | Papel |
|---|---|---|
| `--brand-roxo` | `#644389` | predominante |
...
|---|---|---|---|
| `--bg` | `#0A070E` | `#FAFAFA` | fundo da página |
| `--bg2` | `#130D1A` | `#F5F1FB` | fundo alt / app shell |
| `--card` | `#1B1225` | `#FAF7FE` | superfície de cartão/controle |
| `--card2` | `#22172F` | `#F1EAFA` | superfície elevada/hover |
| `--line` | `rgba(162,137,215,.14)` | `rgba(100,67,137,.16)` | bordas/divisores |
...
### 2.5 Formato do `facial-academy-design-tokens.json`
...
```css
.fc-chip{background:var(--card);border:1px solid var(--line);color:var(--txt);
         padding:9px 16px;border-radius:30px}                 /* neutro, cor via token */
.fc-badge{color:var(--gold-ink);border:1px solid var(--gold-line);font-size:10.5px;
          padding:4px 10px;border-radius:20px}                /* destaque dourado */
.fc-status{padding:9px 15px;border-radius:30px;border:1px solid currentColor}
.fc-status.is-success{color:var(--success);background:var(--success-bg)}
.fc-status.is-warning{color:var(--warning);background:var(--warning-bg)}
.fc-status.is-danger {color:var(--danger);background:var(--danger-bg)}
.fc-status.is-info   {color:var(--info);background:var(--info-bg)}
```
...
### 5.4 Sobreposições
```css
.modal-scrim{position:absolute;inset:0;background:radial-gradient(... rgba(162,137,215,.30) ...),rgba(58,39,79,.55);
...
- **Tema:** init anti-flash + toggle `#themeToggle` (ver 2.1). Chave `localStorage['fc-theme']`.
...
<html lang="pt-BR"><head>
  ...
  <link rel="stylesheet" href="facial-academy-design-system.css">
</head><body>
  <button class="fc-btn fc-fill">Assinar agora</button>
</body></html>
```
...
| Aspecto | Facial Academy | Corporal Class |
|---|---|---|
| Arquivos | `facial-academy-design-system.css` · `facial-academy-design-tokens.json` · `copy-deck.facial-academy.json` | `corporal-design-system.css` · `corporal-design-tokens.json` · `copy-deck.corporal.json` |
| Prefixo de classe (drop-in) | `fc-*` | `cc-*` |
| Chave de tema | `localStorage['fc-theme']` | `localStorage['cc-theme']` |
| id do cmdk | `cmdk-list-fa` | `cmdk-list-co` |
| Marca / Academy | Facial Academy / Facial Academy | Corporal Class / Corporal Academy |
